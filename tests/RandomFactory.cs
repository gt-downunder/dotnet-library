...
namespace Grondo.Tests
{
    public static class RandomFactory
    {
        ...
        {
            string name = personName.RemoveSpecialCharacters().ToLower();
            string company = companyName.RemoveSpecialCharacters().ToLower();

            lock (_randLock)
            {
                string suffix = _emailSuffixes[_random.Next(0, _emailSuffixes.Length)];
